...
> [!NOTICE] reason
> 当在vector上迭代时从vector中删除元素时，迭代器将失效。擦除元素后，指向该元素的迭代器不再有效，如果继续使用它而不更新它，则可能导致意外行为。这就是代码中没有删除3的原因。
...
> [!NOTE] Title
> 没检查出来；
> 不过在`vector<int> vec{1,2,3,4}`这一步之后，该容器的size为4，capacity为4
> 在`erase(4)` 之后size为3，capacity为4；
...
```cpp
vector<int> arr{1, 9, 1, 9, 8, 1, 0};
sort(arr.begin(), arr.end(), greater<int>());//可以没有int
// arr = [9, 9, 8, 1, 1, 1, 0]
```
